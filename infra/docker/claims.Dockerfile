# Claims service image. Build from the repo root so pnpm workspace
# dependencies resolve: docker build -f infra/docker/claims.Dockerfile .
...
EXPOSE 4003
CMD ["pnpm", "--filter", "@sinistria/claims", "start"]
