# Cockpit app image. Build from the repo root so pnpm workspace dependencies
# resolve: docker build -f infra/docker/cockpit.Dockerfile .
...
RUN pnpm --filter @sinistria/cockpit build

ENV NODE_ENV=production
EXPOSE 3001
CMD ["pnpm", "--filter", "@sinistria/cockpit", "start"]
