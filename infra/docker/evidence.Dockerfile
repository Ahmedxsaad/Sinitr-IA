# Evidence service image. Build from the repo root so pnpm workspace
# dependencies resolve: docker build -f infra/docker/evidence.Dockerfile .
...
EXPOSE 4002
CMD ["pnpm", "--filter", "@sinistria/evidence", "start"]
