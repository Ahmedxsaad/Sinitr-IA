# Gateway service image. Build from the repo root so pnpm workspace
# dependencies resolve: docker build -f infra/docker/gateway.Dockerfile .
...
EXPOSE 4000
CMD ["pnpm", "--filter", "@sinistria/gateway", "start"]
