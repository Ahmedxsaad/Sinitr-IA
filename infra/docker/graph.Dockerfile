# Graph service image. Build from the repo root so pnpm workspace
# dependencies resolve: docker build -f infra/docker/graph.Dockerfile .
...
EXPOSE 4004
CMD ["pnpm", "--filter", "@sinistria/graph", "start"]
