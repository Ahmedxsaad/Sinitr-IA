# Intake service image. Build from the repo root so pnpm workspace
# dependencies resolve: docker build -f infra/docker/intake.Dockerfile .
...
EXPOSE 4001
CMD ["pnpm", "--filter", "@sinistria/intake", "start"]
