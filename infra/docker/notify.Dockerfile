# Notify service image. Build from the repo root so pnpm workspace
# dependencies resolve: docker build -f infra/docker/notify.Dockerfile .
...
EXPOSE 4005
CMD ["pnpm", "--filter", "@sinistria/notify", "start"]
